    ...
    model_config = ConfigDict(
        ...
        json_encoders={
            Hypothesis: lambda v: {
                ...
                "concise_reason": v.concise_reason,
                "concise_observation": v.concise_observation,
                "concise_justification": v.concise_justification,
                "concise_knowledge": v.concise_knowledge,
                "problem": v.problem,
                "component": v.component,
            },
            HypothesisFeedback: lambda v: {
                "reason": v.reason,
                "decision": v.decision,
                "exception": v.exception,
                "observations": v.observations,
                "hypothesis_evaluation": v.hypothesis_evaluation,
                "new_hypothesis": v.new_hypothesis
...
class DataScienceV2RDLoop(DataScienceRDLoop):
    ...
    def _write_experiment_manifest(self, exp, step_name):
        ...
        if hasattr(exp, "hypothesis") and exp.hypothesis:
            self.manifest.hypothesis = exp.hypothesis
        if (
            hasattr(exp, "experiment_workspace")
            and exp.experiment_workspace
            and exp.experiment_workspace.workspace_path.exists()
        ):
            self.manifest.workspace_path = str(
                exp.experiment_workspace.workspace_path.relative_to(
            ...
            self.manifest.files = [
                File(
                    name=f.name,
                    path=str(f.relative_to(DS_RD_SETTING.session_path).parent),
                    size=f.stat().st_size,
                )
                for f in exp.experiment_workspace.workspace_path.iterdir()
                if f.is_file()
            ]
                ...
                model_files = [
                    File(
                        ...
                        path=str(f.relative_to(
                            DS_RD_SETTING.session_path).parent),
                        size=f.stat().st_size,
                    )
            ...
            scores = exp.experiment_workspace.workspace_path / "scores.csv"
            if scores.exists():
                self.manifest.metrics = {}
                self.manifest.metrics = pd.read_csv(
                    scores, index_col=0
                ).to_dict()
            
        self._log_object(self.manifest, "manifest", step_name)
        self._log_object(
            self.manifest,
            "manifest",
            step_name,
            Path(DS_RD_SETTING.session_path) / "log" / f"Loop_{self.loop_idx}",
        )
    ...
    def _log_object(
        ...
        if path is None:
            path = (
                Path(DS_RD_SETTING.session_path)
                / "log"
                / f"Loop_{self.loop_idx}"
                / f"{step_name}"
                / f"{name}.json"
            )
        else:
        ...
        with path.open("w") as f:
            try:
                if isinstance(obj, BaseModel):
                    f.write(obj.model_dump_json(exclude_unset=True))
                else:
                    f.write(jsonpickle.encode(obj, unpicklable=True))
            except TypeError:
                logger.error(
                    f"Error in logging object: {name} with type {type(obj)}")
...
class FolderWatcher(FileSystemEventHandler):
    def __init__(self, folder_path, session_id):
        ...
        blob_service_client = BlobServiceClient(
            account_url=DS_RD_SETTING.azure_storage_account_url, credential=credential
        )
        self.container_client = blob_service_client.get_container_client(
            DS_RD_SETTING.azure_storage_container_name
        )
...
def watch_daemon(session_id):
    ...
    thread = threading.Thread(target=watch, args=(
        session_id, stop_event), daemon=True)
    thread.start()
...
def watch(session_id, stop_event):
    ...
    event_handler = FolderWatcher(
        folder_path=folder_to_watch, session_id=session_id)
    observer = Observer()
    ...
    if DS_RD_SETTING.competition:
        if DS_RD_SETTING.scen.endswith("KaggleScen"):
            ...
            if not Path(f"{DS_RD_SETTING.local_data_path}/{competition}").exists():
                logger.error(
                    f"Please prepare data for competition {competition} first."
                )
                return
    else:
...
def initialize_session():
    ...
    RD_AGENT_SETTINGS.workspace_path = Path(
        DS_RD_SETTING.session_path) / "workspace"
